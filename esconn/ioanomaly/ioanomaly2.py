...
def dimenreduction(df,metrics):
    ...
    metrics.insert(0, 'offesthost')
    metrics.insert(0, 'extendhost')
    metrics.insert(0,'mdshost')
    metrics.insert(0, 'endtime')
    metrics.insert(0, 'starttime')
    metrics.insert(0, 'jobid')
    ds = df[metrics]
    lists = ds.values
    metricsvalue_list = lists[:, 7:]
    label = lists[:, 6]
    ...
    print(data.shape)
    decrease_dimension_df = pd.DataFrame(data,columns=['jobid','starttime','endtime','mdshost','extendhost','offesthost','label','dimen1','dimen2'])
