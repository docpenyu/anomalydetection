...
from algorithm.univariate_predictor.SamplePredict import SamplePredictRes
import numpy as np
...
def train(request):
    """
    train model by the selected parameters
    model include LSTM and Iforest
    """
    context = {}
    form = TrainForm()
    context = {'form':form}
        ...
        if obj.is_valid():
            ...
            if(predictmethod == 'None'):
                ...
                if('Iforest' in detectmethod):
                    ...
                    if('timefeature' in detectparam):
                        thiskw['timefeature'] = detectparam['timefeature']
                    ...
                    predictresult = detclass.predictres(model=model)
                    detdf = predictresult['detdf']
                    anomalyscore =  predictresult['Test_Data_Score']
                    anomalylist =  predictresult['anomalylist']
